...
if __name__ == '__main__':
    ...
    date_lst = [('2008-01-01', '2020-12-31')]
    df_lst = []
    lst = []
    state_lst = []
    ...
    for index_code in index_code_lst:
        try:
            ...
            data_daily.to_csv(fold_data + symble[:6] + '.csv')
                            ...
                            try:
                                ...
                                for (s_date, e_date) in date_lst:
                                    ...
                                    print('%s_%s_%s_%s_%s_%s_%s_%s.csv' % (
                                        ...
                                        int(10 * percentile)))
                                    net_df.to_csv(fold + 'net/%s_%s_%s_%s_%s_%s_%s_%s_ls.csv' % (
                                        index_code[:6], method_open, method_close, buy1, buy2, sell1, sell2,
                                        int(10 * percentile)))
    ...
    signal_state = pd.DataFrame(state_lst,
                                columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                         'max_retrace', 'trade_times', 'ave_r', 'ave_hold_days', 'win_r_3',
                                         'win_r_5', 'ave_max', 'percentile', 'buy1', 'buy2', 'sell1',
                                         'sell2', 'method',
                                         's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv(fold + 'state_kdj_macd_idx_ls_' + str(x) + '.csv')
    signal_state_all_df = pd.concat(signal_state_all_lst)
    signal_state_all_df.to_csv(fold + 'signal_kdj_macd_idx_ls_' + str(x) + '.csv')