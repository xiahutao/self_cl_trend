...
if __name__ == '__main__':
    ...
    N1_lst = [i for i in range(6, 8, 10)]  # 短周期周期1
    N2_lst = [i for i in range(29, 32, 10)]  # 长周期周期2
    K1_lst = [i / 100 for i in range(50, 96, 50)]  # 近期比例
    K2_lst = [i / 100 for i in range(30, 96, 500)]  # 长期比例

    P_ATR_LST = [i for i in range(7, 32, 2)]  # ATR周期
    ATR_n_lst = [i / 10 for i in range(5, 32, 2)]  # ATR倍数
    ...
    print(signal_state)
    signal_state.to_csv('E:/data/ocm/' + 'state_ocm_tb_no_add_position' + str(len(symble_lst)) + '.csv')
