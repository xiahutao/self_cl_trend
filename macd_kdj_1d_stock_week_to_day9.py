...
if __name__ == '__main__':
                        ...
                        try:
                            ...
                            for (s_date, e_date) in date_lst:
                                ...
                                net_df.to_csv(fold + 'net/%s_%s_%s_%s_%s_%s_%s_%s.csv' % (
                                    symble[:6], method_open, method_close, buy1, buy2, sell1, sell2,
                                    int(10 * percentile)))
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv(fold + 'state_kdj_macd_' + str(len(index_code_lst)) + '1.csv')
    signal_state_all_df = pd.concat(signal_state_all_lst)
    signal_state_all_df.to_csv(fold + 'signal_kdj_macd_' + str(len(index_code_lst)) + '1.csv')
