...
if __name__ == '__main__':
                    ...
                    for (sell1, sell2) in k2_lst:
                        for percentile in percentile_lst:
                            ...
                            for idx, _row in group_.iterrows():
                                ...
                                condition_s = (_row.k_d_close < 0) and (_row.k_d_close_1 > 0)
                                if pos == 0:
                                    if condition_l:
                            ...
                            try:
                                ...
                                for (s_date, e_date) in date_lst:
                                    ...
                                    print('%s_%s_%s_%s_%s_%s_%s_%s.csv' % (
                                        ...
                                        int(10 * percentile)))
                                    net_df.to_csv(fold + 'net/%s_%s_%s_%s_%s_%s_%s_%s_l.csv' % (
                                        index_code, method_open, method_close, buy1, buy2, sell1, sell2,
                                        int(10 * percentile)))
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv(fold + 'state_kdj_macd_idx_l_' + str(x) + 'adj.csv')
    signal_state_all_df = pd.concat(signal_state_all_lst)
    signal_state_all_df.to_csv(fold + 'signal_kdj_macd_idx_l_' + str(x) + 'adj.csv')