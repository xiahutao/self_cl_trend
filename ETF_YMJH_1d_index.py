...
if __name__ == '__main__':
    ...
    trd_idx_code_list = ['000989', '399550', 'H30165', '399364', '000036', '399973', '000066', '000068', '000048',
                         '000814', '930701', 'H30252', '000063', 'H30251', '000928', '000986', '000987', '399967',
                         '000932', '000056']
    ...
    print('=========len(index_code_lst):', len(index_code_lst))
    ...
    index_code_lst = [i for i in index_code_lst if str(i)[:6] in etf_to_idx.idx_code_trans.tolist()]
    index_code_lst = list(set(index_code_lst))
    print('=========len(index_code_select_lst):', len(index_code_lst))
    index_hq_all = pd.read_csv(fold_data + 'index_hq_index.csv', encoding='gbk')
    index_hq_all = index_hq_all[index_hq_all['trade_date'] >= start_day]\
        .assign(date_time=lambda df: df.trade_date.apply(lambda x: str(x)[:10]))
    print(index_code_lst)

    hq_dict = {}
    for i in range(len(index_code_lst)):
        indus_name = index_code_lst[i]
        hq_df = index_hq_all[index_hq_all['stock_code'] == indus_name]
        hq_dict[indus_name] = hq_df
    # index_hq_dict = []
    # for i in range(len(index_code_lst)):
    #     index_code = index_code_lst[i]
    #     print(index_code)
    #     index_hq = stock_price(index_code, '1d', start_day, end_day)
    #     index_hq_dict.append(index_hq)
    # index_hq_all = pd.concat(index_hq_dict)
    # index_hq_all.to_csv(fold + 'index_hq_index.csv', encoding='gbk')
    n = 1  # 回测周期
    period = '1d'
