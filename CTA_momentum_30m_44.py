...
if __name__ == '__main__':
    ...
    print(os.path)
    t0 = time.time()
    resualt_folder_path = 'E:/data/MOMENTUM/'
    ...
    SK_lst = [(8, 16, 32)]  # 短周期周期1
    LK_lst = [(24, 48, 96)]  # 长周期周期2
    PW = 63
    SW = 252

    P_ATR_LST = [i for i in range(7, 8, 2)]  # ATR周期
    ATR_n_lst = [i / 10 for i in range(1000000, 1000001, 2)]  # ATR倍数
    ...
    for symble in symble_lst:
        group = pd.read_csv('E:/data/hq/' + 'hq_' + period + '_' + symble + '.csv')[
            ['date_time', 'open', 'high', 'low', 'close', 'trade_date']]\
            .assign(date_time=lambda df: df.date_time.apply(lambda x: str(x)[:16]))
        n = len(group[group['trade_date'] == max(group['trade_date'])])
        print(symble, n)
        group_day = pd.read_csv('E:/data/hq/' + 'hq_' + '1d' + '_' + symble + '.csv')[
            ['open', 'high', 'low', 'close', 'date_time']]\
            .assign(trade_date=lambda df: df.date_time.apply(lambda x: x[:10]))
        for N_ATR in P_ATR_LST:
            ...
            day_atr = group_day[['trade_date', 'atr']] \
                .assign(atr=lambda df: df.atr.shift(1)) \
                .merge(group, on=['trade_date'], how='right') \
                .sort_values(['date_time']).fillna(method='ffill')
            # print(day_atr)
            for SK in SK_lst:
                for LK in LK_lst:
                    if len(day_atr) <= max(max(LK), max(SK)):
                        continue
                    Scta = 0
                    for k in range(len(SK)):
                        if SK[k] > LK[k]:
                            break
                        xk = day_atr.close.ewm(span=SK[k]).mean() - day_atr.close.ewm(span=LK[k]).mean()
                        p_std = talib.STDDEV(day_atr.close, timeperiod=PW, nbdev=1)
                        yk = xk/p_std
                        yk_std = talib.STDDEV(yk, timeperiod=SW, nbdev=1)
                        zk = yk/yk_std
                        day_atr['zk'] = zk
                        day_atr['uk'+str(k)] = day_atr.zk.apply(lambda x: x*math.exp(-x*x/4)/0.89)
                    day_atr['Scta'] = 1/3 * (day_atr['uk0'] + day_atr['uk1'] + day_atr['uk2'])
                    group_ = day_atr.assign(Scta=lambda df: df.Scta.shift(1))\
                        .assign(close_1=lambda df: df.close.shift(1))
                    group_[['Scta', 'close']].to_csv(resualt_folder_path + symble + '_df.csv')
                    ...
                    for ATR_n in ATR_n_lst:
                        ...
                        signal_state_all = pd.DataFrame(signal_lst, columns=[
                            ...
                            'position', 'bspk'])
                        # signal_state.to_csv(resualt_folder_path + str(n) + '_' + symble + '_' + back_stime + 'ls.csv')
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'win_r_5', 'ave_max', 'art_N', 'art_n', 'SK', 'LK', 's_time', 'e_time'])
    print(signal_state)
    signal_state.to_csv(resualt_folder_path + 'state_momentum_' + str(len(symble_lst)) + '.csv')
