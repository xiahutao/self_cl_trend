...
if __name__ == '__main__':
    ...
    date_lst = [('2003-01-01', '2020-12-31')]
    df_lst = []
    lst = []
    state_lst = []
    ...
    for index_code in index_code_lst:
        try:
            ...
            # data_daily.to_csv(fold_data + 'A.csv')
                ...
                for (buy1, buy2) in k1_lst:
                    for (sell1, sell2) in k2_lst:
                        for percentile in percentile_lst:
                            ...
                            long_stop = False
                            short_stop = False
                            group_['bk'] = group_[(group_.k_open < buy2) and (group_.d_open > buy1) and (group_.k_d_open > 0) and \
                                              (group_.k_d_open_1 < 0) and (group_.percentile_open <= percentile)]
                            for idx, _row in group_.iterrows():
                                ...
                                condition_sk = (_row.k_open > sell1) and (_row.d_open < sell2) and (_row.k_d_open < 0) and \
                                               (_row.k_d_open_1 > 0) and (_row.percentile_open <= percentile)
                                condition_bp = (_row.k_close < buy2) and (_row.d_close > buy1) and (
                                        _row.k_d_close > 0) and \
                                               (_row.k_d_close_1 < 0) and (_row.percentile_close <= percentile)
                                if pos == 0:
                                    if condition_bk:
                                    ...
                                    if condition_sk:
                                        ...
                                        signal_row.append(ret_hold_days3)
                                        ...
                                        if hold_days == 3:
                                            ret_hold_days3 = _row.close / cost - 1
                                        high_price.append(_row.high)
                                    ...
                                    if condition_bk:
                                        ...
                                        if hold_days <= 3:
                                            ret_hold_days3 = (cost - b_price) / cost
                                        e_time = _row.date_time
                                        trad_times += 1
                                        ...
                                        hold_days = hold_days + 1
                                        if hold_days == 3:
                                            ret_hold_days3 = (cost - _row.close) / cost
                                        low_price.append(_row.low)
                                        hold_price.append(_row.close)
                            ...
                            signal_state_all = pd.DataFrame(signal_lst, columns=[
                                ...
                                'b_price', 's_price', 'ret', 'max_ret', 'hold_day', 'position', 'bspk', 'ret_hold_days3'])
                            ...
                            try:
                                ...
                                for (s_date, e_date) in date_lst:
                                    ...
                                    print('%s_%s_%s_%s_%s_%s_%s_%s.csv' % (
                                    ...
                                    state_row.append(signal_state.ret_hold_days3.mean())
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'sell2', 'method',
                                                    's_time', 'e_time', 'ret_hold_days3'])
    print(signal_state)
    signal_state.to_csv(fold + 'state_kdj_macd_future_' + str(x) + '.csv')
    signal_state_all_df = pd.concat(signal_state_all_lst)
    signal_state_all_df.to_csv(fold + 'signal_kdj_macd_future_' + str(x) + '.csv')