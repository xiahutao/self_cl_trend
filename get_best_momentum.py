...
if __name__ == '__main__':
    ...
    for symble in symble_lst:
        data = pd.read_csv(resualt_folder_path + 'state_momentum_' + '44' + '_N.csv')
        print(data)
    ...
    lst = []
    for method, group in data.groupby(['art_N', 'art_n', 'SK', 'LK']):
        # if len(group)==3:
        #     lst.append(group)
        print(len(group))
    ...
    df = pd.DataFrame(lst, columns=['method', 'num', 'sharp', 'ann_ret', 'trade_times', 'win_r', 'odds', 'max_retrace',
                                    'ave_r', 'ave_hold_days'])
    print(df)
    df.to_csv(resualt_folder_path + 'best_state_momentum_' + period + '_N.csv')
