...
if __name__ == '__main__':
    ...
    date_lst = [('2002-01-01', '2020-12-31')]
    df_lst = []
    lst = []
    state_lst = []
    ...
    for index_code in index_code_lst:
        try:
            ...
            for (method_open, method_close) in method_lst:
                ...
                data['ret_hold3'] = data['close'].shift(-2)/data['close'].shift(1) - 1
                data['ret_hold5'] = data['close'].shift(-4) / data['close'].shift(1) - 1
                data['ret_hold10'] = data['close'].shift(-9) / data['close'].shift(1) - 1
                data = data.dropna()
                    ...
                    for (sell1, sell2) in k2_lst:
                        for percentile in percentile_lst:
                            ...
                            for idx, _row in group_.iterrows():
                                ...
                                condition_sk = (_row.k_open > sell1) and (_row.d_open < sell2) and (
                                            _row.k_d_open < 0) and \
                                               (_row.k_d_open_1 > 0) and (_row.percentile_open <= percentile)
                                ...
                                if condition_bk:
                                    ...
                                    signal_row.append('bk')
                                    signal_row.append(_row.ret_hold3)
                                    signal_row.append(_row.ret_hold5)
                                    signal_row.append(_row.ret_hold10)
                            ...
                            signal_state_all = pd.DataFrame(signal_lst, columns=[
                                'symble', 'percentile', 'buy1', 'buy2', 'sell1', 'sell2', 'method', 'bspk', 'ret_3days',
                                'ret_5days', 'ret_10days'])
    ...
    signal_state_all_df.to_csv(fold + 'ret_hold3days_' + str(x) + 'adj.csv')