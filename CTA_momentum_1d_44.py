...
if __name__ == '__main__':
    ...
    n = 1  # 回测周期
    period = '1d'
    N_lst = [i for i in range(8, 9)]
    SK_lst = []
    LK_lst = []
    for N in N_lst:
        SK_lst.append((N, 2*N, 4*N))
        LK_lst.append((54, 108, 216))
    PW = 63
    SW = 252

    P_ATR_LST = [i for i in range(5, 32, 2)]  # ATR周期
    ATR_n_lst = [i / 10 for i in range(5, 51, 5)]  # ATR倍数
    ...
    date_lst = [('2010-01-01', '2019-10-01')]
    df_lst = []
    lst = []
    state_lst = []
    ...
    print(signal_state)
    signal_state.to_csv(resualt_folder_path + 'state_momentum_' + str(len(symble_lst)) + '_N1.csv')
